...
def reduce_to_manhattan_rectangular(points):
    ...
    min_x, max_x = np.where(points[:, 0] == np.min(points[:, 0]))[
        0], np.where(points[:, 0] == np.max(points[:, 0]))[0]
    min_y, max_y = np.where(points[:, 1] == np.min(points[:, 1]))[
        0], np.where(points[:, 1] == np.max(points[:, 1]))[0]
...
def check_if_point_inside_a_rectangular(quad, query):
    ...
    """
    AB, AM, BC, BM = quad[1]-quad[0], query - \
        quad[0], quad[2]-quad[1], query-quad[1]
...
def check_if_point_inside_conductors(conductors, q):
    ...
    left, right = position_of_point_towards_set_of_lines(
        conductors[:, 3], conductors[:, 0], q), position_of_point_towards_set_of_lines(conductors[:, 2], conductors[:, 1], q)
    # Compute the position of the query towards horizontal lines of conductors
    top, bottom = position_of_point_towards_set_of_lines(
        conductors[:, 1], conductors[:, 0], q), position_of_point_towards_set_of_lines(conductors[:, 2], conductors[:, 3], q)
...
def check_if_point_between_parallels_of_set_of_conductors(conductors, q):
    ...
    between_verticals = np.where(
        (point_positions[0]*point_positions[1]) < 0)[0]
    # point is between the parallels of the horizontal line segments for conductors
    between_horizontal = np.where(
        (point_positions[2]*point_positions[3]) < 0)[0]
    # Keep lenghts of each case
    len_between_vert, len_between_hor = len(
        between_verticals), len(between_horizontal)
    # Encode the position information
    if len_between_vert:
        point_positions_encoded[between_verticals] = np.array([np.ones(
            (len_between_vert)), point_positions[2][between_verticals], point_positions[3][between_verticals]]).T
    if len(between_horizontal):
        point_positions_encoded[between_horizontal] = np.array([np.zeros(
            (len_between_hor)), point_positions[0][between_horizontal], point_positions[1][between_horizontal]]).T
    ...
    while nodes:
        ...
        for node_number, quad in enumerate(nodes):
            ...
            if len(nodes_parent):
                ...
                quadrants_info[nodes_parent[node_number][0]
                               ][nodes_parent[node_number][1]] = q_ind
...
def random_query_points(ordered_vertices, quadrants, quadrants_info, ordered_conductors, n_points):
    """
    ...
    xmin, xmax, ymin, ymax = np.min(ordered_vertices[:, 0]), np.max(
        ordered_vertices[:, 0]), np.min(ordered_vertices[:, 1]), np.max(ordered_vertices[:, 1])
    ...
    while len(random_points)-n_points:
        ...
        random_point = [xmin+(xmax-xmin)*random.random(),
        ...
        print(
            f'Have been created {len(random_points)}/{n_points} random query points.', end='\r')
...
def get_quadrant_borders(indexes_of_infertile_quadrants, quadrants_info, quadrant_vertices):
    ...
    for w in sorted_widths:
        ...
        quadrants_per_width_collection.append(
            np.where(np.array(quadrants_info)[:, 2] == w)[0])
        # Save the vertices collection of same width quadrants into your list.
        quadrants_vertices_per_width_collection.append(np.reshape(
            quadrant_vertices[quadrants_per_width_collection[-1]], (len(quadrants_per_width_collection[-1])*4, 2)))
    ...
    for q_ind, ind in enumerate(indexes_of_infertile_quadrants):
        ...
        while not np.all(quadrants_borders_info[ind]):
            ...
            same_q_vertices = quadrants_vertices_per_width_collection[
                quadrant_width_index+next_width]
            ...
            q_info, q_vertices = quadrants_info[q_info[4]
                                                ], quadrant_vertices[q_info[4]]
        # Print out update
        print(
            f'Borders of quadrant number {q_ind}/{len_of_infertile_quadrants} have been explored.', end='\r')
...
def compute_mec(quadrant, ordered_conductors, qp):
    ...
    mecs = []
    # Collect quadrant's_conductors
    quadrant_conductors = ordered_conductors[quadrant.unique_conductors_indexes]
    # Find the closest vertex of each conductor to the query point.
    shortest_vertices = np.argmin(np.linalg.norm(
        quadrant_conductors-qp, axis=2), axis=1)
    # Check if the point is in between the parallels of the conductors (not inside the conductor!!!)
    point_positions = check_if_point_between_parallels_of_set_of_conductors(
        quadrant_conductors, qp)
    # For unique conductor in quadrant
    for cond_num, conductor in enumerate(quadrant_conductors):
...
def search_borders_conductors(qp, ordered_conductors, quadrant_vertices, quadrants, quadrants_borders, quad_ind,
    ...
    border_distances = qp_distance_from_borders(
        quadrant_vertices[quad_ind], qp)
    ...
    for border_ind in sorted_border_distances_indexes:
        ...
        if border_distances[border_ind] < mec_distance:
            ...
            border_mecs = compute_mec(
                neighbor_quadrant, ordered_conductors, qp)
            ...
            neighbors_closest_conductor, neighbors_mec_distance = neighbor_quadrant.unique_conductors_indexes[
                min_borders_mec], border_mecs[min_borders_mec]
...
class Quadrant():
    ...
    def __init__(self, x0, y0, w, h, line_points):
        ...
        self.unique_conductors_indexes = np.unique(
            [line.idx for line in self.lines]).astype(int)
        self.children = []
...
class QTree():
    ...
    def __init__(self, lines, k, root_node):
        ...
        # Quadrant(0, 0, 50, 50, self.lines)
        self.root = Quadrant(*root_node, self.lines)
...
side_corner_to_quadrant_border = {0: (0, 1), 1: (0, 2), 2: (
    0, 3), 3: (1, 3), 4: (1, 0), 5: (2, 0), 6: (2, 1), 7: (3, 1)}


class QIndex():
    def __init__(self, data, qtree_threshold=40):
        self.data = data
        self.qtree_threshold = qtree_threshold
        self.ordered_conductors = self.cube_vertices()
        self.ordered_vertices = self.order_vertices()
        self.lines = self.create_lines()
        self.xmin, self.xmax, self.ymin, self.ymax = self.get_min_max()
        self.w, self.h = self.get_width_height()
        self.quadtree = self.create_quadtree()
        self.quadrants, self.quadrants_info = self.quadrants_relationships()
        self.quadrant_vertices = self.get_quadrants_vertices()
        self.quadrants_borders = self.get_quadrant_borders()

    def cube_vertices(self):
        v1_rad, v2_rad, v3_rad = np.pi - \
            (self.data[:, -1]+(np.pi/2)), self.data[:, -1] + \
            (np.pi/4), self.data[:, -1]
        cube_diagonal = (2*self.data[:, -2]**2)**0.5
        ordered_conductors = np.zeros((len(self.data), 4, 2))
        v1_rel = np.array([np.cos(v1_rad)*self.data[:, -2],
                          np.sin(v1_rad)*self.data[:, -2]]).T
        v2_rel = np.array([np.cos(v2_rad)*cube_diagonal,
                          np.sin(v2_rad)*cube_diagonal]).T
        v3_rel = np.array([np.cos(v3_rad)*self.data[:, -2],
                          np.sin(v3_rad)*self.data[:, -2]]).T
        ordered_conductors[:, 0] = np.array(
            [self.data[:, 0]-v1_rel[:, 0], self.data[:, 1]+v1_rel[:, 1]]).T
        ordered_conductors[:, 1] = np.array(
            [self.data[:, 0]+v2_rel[:, 0], self.data[:, 1]+v2_rel[:, 1]]).T
        ordered_conductors[:, 2] = np.array(
            [self.data[:, 0]+v3_rel[:, 0], self.data[:, 1]+v3_rel[:, 1]]).T
        ordered_conductors[:, 3] = self.data[:, :-2]
        return ordered_conductors

    def order_vertices(self):
        return np.reshape(self.ordered_conductors, (self.ordered_conductors.shape[0]*self.ordered_conductors.shape[1], self.ordered_conductors.shape[2]))

    def create_lines(self):
        lines = []
        for i, quad in enumerate(self.ordered_conductors):
            for s in range(4):
                lines.append(list(quad[s+0])+list(quad[s-3])+[i])
        return np.array(lines)

    def get_min_max(self):
        return np.min(self.ordered_vertices[:, 0]), np.max(self.ordered_vertices[:, 0]), np.min(self.ordered_vertices[:, 1]), np.max(self.ordered_vertices[:, 1])

    def get_width_height(self):
        return self.xmax-self.xmin, self.ymax-self.ymin

    def create_quadtree(self):
        quadtree = QTree(self.lines, self.qtree_threshold,
                         (self.xmin, self.ymin, self.w, self.h))
        quadtree.subdivide()
        return quadtree

    def save(self):
        print("Saving quadtree...")
        # needs work
        # Swith open(f'./data/quadtree_{self.qtree_threshold}_{self.data}_NM.pickle', 'wb') as file:
        #pickle.dump(self.quadtree, file)

    def quadrants_relationships(self):
        nodes, nodes_parent = [self.quadtree.root], []
        quadrants, quadrants_info = [], []
        q_ind = 0
        while nodes:
            new_nodes = []
    ...
    def get_quadrants_vertices(self):
        quadrant_vertices = np.zeros((len(self.quadrants_info), 4, 2))
        for i, quadrant in enumerate(self.quadrants_info):
            x, y, w, h = quadrant[:4]
            quadrant_vertices[i] = [[x, y+h], [x+w, y+h], [x+w, y], [x, y]]
        return quadrant_vertices

    def get_quadrant_borders(self):
        indexes_of_infertile_quadrants = np.where(
            np.array(self.quadrants_info)[:, 6] == -1)[0]
        quadrants_borders_info = np.zeros(
            (len(self.quadrants_info), 4, 4), dtype=int)
        len_of_infertile_quadrants = len(indexes_of_infertile_quadrants)
        sorted_widths = np.sort(np.unique(np.array(self.quadrants_info)[:, 2]))
        len_of_widths = len(sorted_widths)
        quadrants_per_width_collection, quadrants_vertices_per_width_collection = [], []
        for w in sorted_widths:
            quadrants_per_width_collection.append(
                np.where(np.array(self.quadrants_info)[:, 2] == w)[0])
            quadrants_vertices_per_width_collection.append(np.reshape(
                self.quadrant_vertices[quadrants_per_width_collection[-1]], (len(quadrants_per_width_collection[-1])*4, 2)))
        for q_ind, ind in enumerate(indexes_of_infertile_quadrants):
            q_info, q_vertices = self.quadrants_info[ind], self.quadrant_vertices[ind]
            next_width = 0
            quadrant_width_index = np.where(sorted_widths == q_info[2])[0][0]
            while not np.all(quadrants_borders_info[ind]):
                if len_of_widths <= quadrant_width_index+next_width:
                ...
                for i, ver in enumerate(q_vertices):
                    ...
                    for vertex in touch:
                        ...
                        if not quadrants_borders_info[ind][i][vertex_position]:
                            quadrants_borders_info[ind][i][vertex_position] = same_q_indexes[ssq_ind]
                next_width += 1
                q_info, q_vertices = self.quadrants_info[q_info[4]
                                                         ], self.quadrant_vertices[q_info[4]]
            #print(f'Borders of quadrant number {q_ind}/{len_of_infertile_quadrants} have been explored.', end='\r')
        return quadrants_borders_info


def nmann(qindex, query_points):
    qp_min_mec = []
    qp = query_points
    quad_ind = find_quadrant(qindex.quadrants_info, qp)
    central_quadrant = qindex.quadrants[quad_ind]
    if not len(central_quadrant.unique_conductors_indexes):
        mec_distance, closest_conductor = np.inf, -1
        closest_conductor, mec_distance = search_borders_conductors(qp, qindex.ordered_conductors, qindex.quadrant_vertices, qindex.quadrants, qindex.quadrants_borders,
                                                                    quad_ind, mec_distance, closest_conductor, side_corner_to_quadrant_border)
    else:
        mecs = compute_mec(central_quadrant, qindex.ordered_conductors, qp)
        min_mec = np.argmin(mecs)
        closest_conductor, mec_distance = central_quadrant.unique_conductors_indexes[
            min_mec], mecs[min_mec]
        closest_conductor, mec_distance = search_borders_conductors(qp, qindex.ordered_conductors, qindex.quadrant_vertices, qindex.quadrants, qindex.quadrants_borders,
                                                                    quad_ind, mec_distance, closest_conductor, side_corner_to_quadrant_border)
    qp_min_mec.append([closest_conductor, mec_distance])
    return qp_min_mec
